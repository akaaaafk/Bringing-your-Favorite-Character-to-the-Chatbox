...
SELECTED_CHARACTERS_PATH = ROOT / "data" / "processed" / "selected_characters.json"
WEB_DIST = ROOT / "web" / "dist"

# Fallback if selected_characters.json is missing (matches inference.py)
...
def load_characters() -> list[dict]:
    ...
    if SELECTED_CHARACTERS_PATH.exists():
        return json.loads(SELECTED_CHARACTERS_PATH.read_text(encoding="utf-8"))
    return [
        {
            "persona_tag": tag,
...
async def tts(req: TtsRequest):
    ...
    tag = normalize_character(req.character)
    try:
        from tts_edge import iter_mp3_chunks
