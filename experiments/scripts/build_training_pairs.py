...
Cornell conversation. Output is used by experiments/scripts/train_generator.py.

Reads:
    config/personas.json
    data/processed/lines.csv          (from experiments/scripts/preprocess.py)
...
def main() -> None:
    ...
    if not lines_path.exists():
        raise FileNotFoundError(
            f"{lines_path} not found. Run experiments/scripts/preprocess.py first."
        )
    if not conv_path.exists():
        raise FileNotFoundError(
            f"{conv_path} not found. Run experiments/scripts/download_data.py first."
        )
