...
Reads:
    ...
    config/personas.json                  (character_name -> persona_tag)

Writes:
    models/classifier/                    (HF checkpoint — auto-detected by
                                            movie_persona.classifier,
                                            which is what Phase 3 imports)
    results/published/<run_name>_metrics.json
    results/published/<run_name>_confusion_matrix.png

Usage:
    python experiments/scripts/train_classifier.py                  # real + synthetic
    python experiments/scripts/train_classifier.py --no-synthetic   # ablation
"""
...
RESULTS_DIR = ROOT / "results" / "published"
...
def build_split(
    ...
    if len(unmapped):
        ...
        raise ValueError(
            ...
            f"{bad_names} not present in config/personas.json. "
            "Check that the persona manifest is up to date with your "
...
def main() -> None:
    ...
    # Production inference loads models/classifier/.
    ...
    # Save model and tokenizer to the production classifier artifact path.
